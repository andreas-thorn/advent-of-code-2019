...
def run(input_value, input_data=None):
    ...
    while index < len(program):
        ...
        def get_operand_index(mode, operand_idx):
            if mode == 0:  # position mode
                return program[index + operand_idx]
            elif mode == 1:  # immediate mode
                return index + operand_idx
            elif mode == 2:  # relative mode
                return program[index + operand_idx] + relative_base
        ...
        if any(operand_index > len(program) for operand_index in [operand_1, operand_2, operand_3]):
            program += [0] * (max([operand_1, operand_2, operand_3]) + 1)  # Extend program with zeros

        if op_code == 1:
            program[operand_3] = program[operand_1] + program[operand_2]
            index += 4
        elif op_code == 2:
            program[operand_3] = program[operand_1] * program[operand_2]
            index += 4
        elif op_code == 3:
            program[operand_1] = input_value
            index += 2
        elif op_code == 4:
            print(program[operand_1])
            index += 2
        elif op_code == 5:
            index = program[operand_2] if program[operand_1] != 0 else index + 3
        elif op_code == 6:
            index = program[operand_2] if program[operand_1] == 0 else index + 3
        elif op_code == 7:
            program[operand_3] = 1 if program[operand_1] < program[operand_2] else 0
            index += 4
        elif op_code == 8:
            program[operand_3] = 1 if program[operand_1] == program[operand_2] else 0
            index += 4
        elif op_code == 9:
            relative_base += program[operand_1]
            index += 2
